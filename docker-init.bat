...
%COMPOSE_CMD% --env-file "%BACKEND_DIR%\.env" -f docker-compose.yml build
if %errorlevel% neq 0 (
...
%COMPOSE_CMD% --env-file "%BACKEND_DIR%\.env" -f docker-compose.yml up -d
if %errorlevel% neq 0 (
...
%COMPOSE_CMD% --env-file "%BACKEND_DIR%\.env" -f docker-compose.yml exec -T -u root app composer install --no-interaction --prefer-dist --no-progress
if %errorlevel% neq 0 (
...
%COMPOSE_CMD% --env-file "%BACKEND_DIR%\.env" -f docker-compose.yml exec -T app php artisan key:generate --force
if %errorlevel% neq 0 (
...
%COMPOSE_CMD% --env-file "%BACKEND_DIR%\.env" -f docker-compose.yml exec -T app php artisan migrate --force
if %errorlevel% neq 0 (
...
%COMPOSE_CMD% --env-file "%BACKEND_DIR%\.env" -f docker-compose.yml exec -T app php artisan db:seed --force
if %errorlevel% neq 0 (
...
%COMPOSE_CMD% --env-file "%BACKEND_DIR%\.env" -f docker-compose.yml exec -T app php artisan config:cache
if %errorlevel% neq 0 (
    echo Aviso: Nao foi possivel gerar o cache de configuracao (config:cache).
)
%COMPOSE_CMD% --env-file "%BACKEND_DIR%\.env" -f docker-compose.yml exec -T app php artisan route:cache

echo - Verificando link simbolico de storage...
%COMPOSE_CMD% --env-file "%BACKEND_DIR%\.env" -f docker-compose.yml exec -T app php artisan storage:link 2>nul

echo - Ajustando permissoes de storage e cache...
%COMPOSE_CMD% --env-file "%BACKEND_DIR%\.env" -f docker-compose.yml exec -T app sh -c "chmod -R 775 storage bootstrap/cache 2>/dev/null || true"
%COMPOSE_CMD% --env-file "%BACKEND_DIR%\.env" -f docker-compose.yml exec -T app sh -c "chown -R www-data:www-data storage bootstrap/cache 2>/dev/null || true"
